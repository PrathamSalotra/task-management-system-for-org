...
export async function addProjectMemberHandler(
  ...
  const validationResult = addProjectMemberSchema.safeParse(req.body);
  ...
  try {
    ...
    console.error('Error in addProjectMemberHandler:', err);
    res.status(500).json({
      error: 'Internal server error',
    });
  }
...
export async function removeProjectMemberHandler(
  ...
  try {
    ...
    if (err.message && err.message.includes('Member not found')) {
      res.status(404).json({
        error: err.message,
      });
      return;
    ...
    console.error('Error in removeProjectMemberHandler:', err);
    res.status(500).json({
      error: 'Internal server error',
    });
  }
